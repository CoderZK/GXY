...
@interface GGXXYYGoodDetailTVC ()
@property(nonatomic,strong)UIView *headV,*footV;
@property(nonatomic,strong)UILabel *numberLB;
@end

@implementation GGXXYYGoodDetailTVC
...
- (void)setFootV {
    ...
    [bt1 setImage:[UIImage imageNamed:@"GGXXYYjian"] forState:UIControlStateNormal];
    bt1.tag = 100;
    ...
    [bt2 setImage:[UIImage imageNamed:@"jia"] forState:UIControlStateNormal];
    bt2.tag = 101;
...
- (void)clickAction:(UIButton *)button {
    ...
    if (button.tag == 100) {
        ...
        NSString * sql = [NSString stringWithFormat:@"insert into kk_mygoodscar (userName,des,price,number,shangpinurl,goodId,desTwo) values ('%@','%@','%f','%ld','%@','%ld','%@')",[GGXXYYSignleTool shareTool].session_uid,self.model.des,self.model.price,(long)number,self.model.img,(long)self.model.ID,self.model.desTwo];
