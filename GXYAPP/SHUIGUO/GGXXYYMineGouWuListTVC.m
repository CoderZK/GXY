//
//  GGXXYYMineGouWuListTVC.m
//  SUNWENTAOAPP
//
//  Created by GGXXYY on 2018/12/20.
...
@interface GGXXYYMineGouWuListTVC ()
@property(nonatomic , strong)NSMutableArray<GGXXYYHomeModel *> *dataArray;
@end

@implementation GGXXYYMineGouWuListTVC
...
- (void)getData {
    ...
    NSString * sql = [NSString stringWithFormat:@"select *from kk_mygoodscar where userName = '%@' and status = 1",[GGXXYYSignleTool shareTool].session_uid];
...
- (UITableViewCell * )tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.dataArray[indexPath.row].isSelect) {
        [cell.bt setBackgroundImage:[UIImage imageNamed:@"GGXXYY_xuanzhong"] forState:UIControlStateNormal];
    }else {
        [cell.bt setBackgroundImage:[UIImage imageNamed:@"GGXXYY_weixuanzhong"] forState:UIControlStateNormal];
    }
