...
@interface FYGoodDetailTVC ()
@property(nonatomic,strong)UIView *headV,*footV;
@property(nonatomic,strong)UILabel *numberLB;
@end

@implementation FYGoodDetailTVC
...
- (void)setFootV {
    ...
    [bt1 setImage:[UIImage imageNamed:@"FYjian"] forState:UIControlStateNormal];
    bt1.tag = 100;
...
- (void)clickAction:(UIButton *)button {
    ...
    if (button.tag == 100) {
        ...
        NSString * sql = [NSString stringWithFormat:@"insert into kk_mygoodscar (userName,des,price,number,shangpinurl,goodId,desTwo) values ('%@','%@','%f','%ld','%@','%ld','%@')",[FYSignleTool shareTool].session_uid,self.model.des,self.model.price,(long)number,self.model.img,(long)self.model.ID,self.model.desTwo];
