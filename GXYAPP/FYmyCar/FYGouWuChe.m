...
//#import "FYJieSuanVC.h"
#import "FYSuanVC.h"
@interface FYGouWuChe ()
/**  */
...
- (void)qiangdanAction:(UIButton *)button {
    ...
    FYSuanVC * vc =[[FYSuanVC alloc] init];
...
- (void)getData {
    ...
    NSString * sql = [NSString stringWithFormat:@"select *from kk_mygoodscar where userName = '%@' and status = 0",[FYSignleTool shareTool].session_uid];
...
- (void)updateFootV {
    CGFloat allmoey = 0;
    NSMutableArray<FYHomeModel *> * arr = @[].mutableCopy;
    for (FYHomeModel * model  in self.dataArray) {
        if (model.isSelect) {
