...
#import "GGXXYYMyCarCell.h"
//#import "GGXXYYJieSuanVC.h"
#import "GGXXYYSuanVC.h"
@interface GGXXYYGouWuChe ()
/**  */
...
@property(nonatomic , strong)NSMutableArray<GGXXYYHomeModel *> *itemArr;
@end

@implementation GGXXYYGouWuChe
...
- (void)viewDidLoad {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"GGXXYYMyCarCell" bundle:nil] forCellReuseIdentifier:@"cell"];
    
    self.tableView.frame = CGRectMake(0, 0, ScreenW, ScreenH - 60-49-34);
    
    [self createFootV];
}

- (void)createFootV {
    self.footV = [[UIView alloc] initWithFrame:CGRectMake(0, ScreenH - 60 - sstatusHeight - 44-49-34, ScreenW, 60)];
...
- (void)qiangdanAction:(UIButton *)button {
    ...
    GGXXYYSuanVC * vc =[[GGXXYYSuanVC alloc] init];
...
- (void)getData {
    ...
    NSString * sql = [NSString stringWithFormat:@"select *from kk_mygoodscar where userName = '%@' and status = 0",[GGXXYYSignleTool shareTool].session_uid];
...
- (UITableViewCell * )tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    GGXXYYMyCarCell * cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
...
- (void)updateFootV {
    CGFloat allmoey = 0;
    NSMutableArray<GGXXYYHomeModel *> * arr = @[].mutableCopy;
    for (GGXXYYHomeModel * model  in self.dataArray) {
        if (model.isSelect) {
